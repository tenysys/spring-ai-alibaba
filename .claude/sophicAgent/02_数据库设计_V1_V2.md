...
#### `sophic_agent_application`
- 表说明：应用主表，承载通用智能体和专用智能体。
- 关键属性：
- `agent_id`：应用业务标识。
- `agent_code`：应用编码。
- `name`：应用名称。
- `description`：应用说明。
- `agent_scope`：应用范围，区分通用/场景化。
- `scene_type`：业务场景类型。
...
- `version`：版本号。
- `version_desc`：版本说明。
- `config_snapshot`：应用配置快照。
- `workflow_definition_id`：关联工作流定义。
- `runtime_config`：运行配置。
- `status`：版本状态。
...
- 表说明：工作流版本表。
- 关键属性：
- `definition_id`：所属工作流定义。
- `version`：版本号。
- `version_desc`：版本说明。
- `definition_snapshot`：流程定义快照。
- `status`：状态。

#### `sophic_agent_workflow_node`
- 表说明：工作流节点表。
- 关键属性：
- `definition_id` / `version`：所属流程版本。
- `node_id`：节点标识。
- `node_type`：节点类型。
- `node_name`：节点名称。
- `node_config`：节点配置。
...
- `source_handle` / `target_handle`：句柄信息。
- `edge_config`：连线配置。
...
- `agent_id`：应用标识。
- `agent_version`：应用版本。
- `resource_type`：资源类型，如模型、知识库、工具、数据源、记忆库等。
- `resource_id`：资源业务标识。
- `resource_name`：资源名称。
- `binding_source`：绑定来源。
- `sort_no`：排序。
- `enabled`：启用状态。
- `binding_config`：绑定配置。
- `selector_config`：选中范围配置。

#### `sophic_agent_application_binding_item`
- 表说明：应用绑定明细表。
- 关键属性：
- `binding_id`：所属绑定关系。
- `item_type`：明细类型，如 MCP 工具、表、字段、概念等。
- `item_id`：明细项标识。
- `item_name`：名称。
- `item_config`：明细配置。
- `sort_no`：排序。
