...
- 已有复用表沿用原系统主键实现，例如 `users.ID varchar(128)`、`priv_group.id varchar(32)`、`priv_role.id varchar(32)`、`smc_application.application_id varchar(32)`、`smc_service.id varchar(64)`。
- 个别业务表允许按领域约定使用差异化字段命名，例如 `sa_datasource.datasource_status`、`sa_datasource.gmt_create_time`、`sa_datasource.gmt_update_time`。
...
- 账号权限域已有表沿用原系统定义，主键主要为字符型主键。
- `users.ID`、`user_info.USER_ID`、`user_additional_info.user_id`、`user_login.USER_ID`、`user_sign.user_id` 使用 `varchar(128)`。
- `priv_group.id`、`priv_role.id`、`priv_permission.id`、`priv_menu_resource.id`、`priv_menu_access.per_id` 使用 `varchar(32)`。
- `priv_group_role_relation`、`priv_role_permission_relation`、`priv_user_group_relation` 使用联合主键，不额外引入自增主键。
- `smc_application.application_id` 使用 `varchar(32)`，`smc_service.id` 使用 `varchar(64)`，且 `smc_service.ref_application` 外键关联 `smc_application.application_id`。
- 已有表中的既有索引继续复用，如 `user_info.phone_index`、`priv_role.name_index`、`priv_menu_access.menu_index`。
...
| `sa_datasource` | 数据源 | PK:`id` UQ:`datasource_id` |
...
- 索引收敛原则
  - 单表索引优先控制在 `3~5` 组以内，超出需有明确查询场景支撑。
  - `status`、`enabled`、`type` 等低选择性字段不单独建索引，优先作为复合索引后缀字段。
  - 高频写入表优先保留“业务唯一键 + 1~2 组核心复合索引”，避免堆叠单列索引。
  - 索引以真实查询路径为准，不把所有可筛选字段都转成单列索引。
- 应用查询类
...
```mermaid
erDiagram
    users ||--o{ priv_user_group_relation : belongs
    priv_group ||--o{ priv_user_group_relation : contains
    priv_group ||--o{ priv_group_role_relation : grants
    priv_role ||--o{ priv_group_role_relation : assigned
    priv_role ||--o{ priv_role_permission_relation : owns
    priv_permission ||--o{ priv_role_permission_relation : mapped
    priv_permission ||--o{ priv_menu_access : controls
    priv_menu_resource ||--o{ priv_menu_access : exposes
    sa_resource ||--o{ sa_resource_acl : authorizes
```
...
```mermaid
erDiagram
    sa_agent_template ||--o{ sa_application : initializes
    sa_application ||--o{ sa_application_version : has
    sa_application ||--o{ sa_application_binding : binds
    sa_application_binding ||--o{ sa_application_binding_item : selects
    sa_application ||--o{ sa_guardrail_rule : protects
    sa_guardrail_rule ||--o{ sa_guardrail_fixed_reply : links
    sa_application ||--o{ sa_publish_record : publishes
    sa_resource ||--o{ sa_publish_record : targets
    sa_high_code_service ||--o{ sa_high_code_service_version : has
    sa_high_code_service ||--o{ sa_publish_record : publishes
```
...
```mermaid
erDiagram
    users ||--o{ sa_session : opens
    sa_application ||--o{ sa_session : serves
    sa_session ||--o{ sa_session_message : contains
    sa_session ||--o{ sa_task : triggers
    sa_session_message ||--o| sa_task : request
    sa_task ||--o| sa_session_message : response
    sa_task ||--o{ sa_execution_step : expands
    sa_high_code_service ||--o{ sa_high_code_deploy_record : deploys
    sa_high_code_service ||--o{ sa_high_code_operation_log : operates
```
...
```mermaid
erDiagram
    sa_knowledge_base ||--o{ sa_knowledge_document : contains
    sa_knowledge_document ||--o{ sa_knowledge_chunk : splits
    sa_tool ||--o{ sa_tool_version : has
    sa_tool ||--o{ sa_tool_debug_record : debugs
    sa_mcp_server ||--o{ sa_mcp_server_instance : runs
    sa_memory_library ||--o{ sa_memory_rule : defines
    sa_memory_library ||--o{ sa_memory_long_term : stores
    sa_memory_library ||--o{ sa_memory_entity : archives
    sa_memory_long_term ||--o{ sa_memory_hit_record : hits
    sa_memory_library ||--o{ sa_memory_recall_test : tests
    sa_datasource ||--o{ sa_datasource_table : contains
    sa_datasource_table ||--o{ sa_datasource_field : contains
    sa_datasource ||--o{ sa_datasource_relation : relates
    sa_datasource ||--o{ sa_datasource_semantic_model : describes
```
...
```mermaid
erDiagram
    sa_model_provider ||--o{ sa_model : provides
    sa_model ||--o{ sa_model_config : configures
    sa_task ||--o{ sa_invoke_log : logs
    sa_application ||--o{ sa_invoke_log : invokes
    sa_application ||--o{ sa_agent_feedback : receives
    sa_task ||--o{ sa_agent_feedback : references
    sa_eval_dataset ||--o{ sa_eval_dataset_item : contains
    sa_eval_dataset ||--o{ sa_eval_task : drives
    sa_eval_task ||--o{ sa_eval_result : outputs
    sa_eval_dataset_item ||--o{ sa_eval_result : scores
    sa_tag ||--o{ sa_tag_binding : binds
```
